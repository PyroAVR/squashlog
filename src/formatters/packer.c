...
#endif
    if(size > 1) {
        buf_write(ctx->buffer, (char*)&type_flag, 1);
        size--;
    }
    if(size) {
        buf_write(ctx->buffer, (char*)&value, size);
    }
    return size;
}
...
int sq_pack_r(fmt_ctx_t *ctx, void *spec, va_list ap) {
    ...
    size_t bytes_avail = buf_get_free(ctx->buffer);
    // further refinement of this value follows
    int bytes_needed = ctx->next_arg_bytes(ctx->format_ctx, spec);
    ...
    while(bytes_needed != 0) {
        if(bytes_needed == -1) {
            ...
            if((size_t)bytes_needed < bytes_avail) {
                bytes_avail -= buf_write(ctx->buffer, arg.as_ptr[0], bytes_needed);
            }
        }
...
// TODO these macros only work on gcc-esque compilers.
                ...
                default:
                    arg.as_max_size_type = va_arg(ap, BINFMT_MAX_TYPE);
                break;
            }
